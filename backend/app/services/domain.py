...
ENGINE_DIGEST_PROMPT = """# 角色
你是一位善于阅读和总结的助手，能把多场内容理解透并浓缩成一份结构化汇总。

# 任务
阅读下面多条来源的章节要点（每条只含 author 和 chapters）。把相同主题合并到同一板块，再把摘要写入 JSON 的 overview。

# 理解步骤
1. 按主题归类：不同来源里讲同一件事的章节并成同一板块。不要按来源拆章，不要一来源一个 ###。
2. 顶层 ### 做成 3 到 6 个主题板块，写成「一、……」「二、……」。
3. 某一板块内容多时，再在其下用 ####「1.」「2.」拆 2 到 5 个小节。只有 1 个子板块时不要写 ####，正文直接放在 ### 下。
4. 抓核心：这些来源合在一起到底在讲什么，写成一句能独立成立的判断。
5. 拆证据：各板块用关键事件、数据、案例支撑；事件自身的日期/盘中时刻写进表的「事件时间」。
6. 辨立场：区分客观事实、讲者主张、修辞/类比包装，以及讲者自己承认的边界。
7. 关键判断旁用括号注明来源作者（抄输入里的 author；作者为空则写「未署名」）。同一判断被多名作者提到时可并列。
...
## 论证结构
板块必须编号。顶层 ### 用汉字「一、二、三」；子板块 #### 用阿拉伯数字「1. 2. 3.」。禁止写片子时钟（hh:mm:ss）。
...
「事件时间」只写输入里该事件自己的日期或盘中时刻。没有就写「未注明」。禁止把音视频播放进度写进表格或标题。

## 辨立场
先肯定原文里可操作的纪律或方法（必须能在输入章节中找到）。再指出：修辞类比与严谨论证的区别、方法边界（仅当原文提到）、信息来源是否只来自讲者。不要编造外部资料。

# 原则
1. 严格忠于输入章节，禁止编造不存在的信息。总结不是摘抄，要用自己的话改写压缩。
2. 事实和观点分开，不混为一谈。
3. 禁止片子时钟。关键数字注明有无来源；输入没给出就写「未注明」。
4. 保持精炼，不要多余开场白。大板块下只有 1 个子板块时不要重复写子标题。
5. 不要输出 chapters 或 key_points。必须使用简体中文。
6. overview 必须写满。禁止用「...」「……」「Markdown 正文，格式见下」或只留标题骨架。主题表三行、每个板块和辨立场都要写成完整句子。
"""
...
def digest_prompt(pack: DomainPack | None = None) -> str:
    current = pack or load_active_pack()
    body = ENGINE_DIGEST_PROMPT
    role = (current.overview_role or "").strip()
    if role:
        marker = "# 角色\n"
        start = body.find(marker)
    ...
    extras = _join_extras(
        [
            current.overview_stance,
            current.term_aliases,
            current.disclaimer,
            "这是多场定时汇总，不是单场音视频。禁止写片子时钟。相同主题必须合并进同一板块，并在判断旁注明来源作者。",
        ]
    )
    if extras:
        return body.rstrip() + extras
