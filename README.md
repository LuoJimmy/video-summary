...
![创建任务](http://my-images-space.oss-cn-shenzhen.aliyuncs.com/video-summay/task_list.png)
...
6. 在「设置 → 定时拉取」打开每天扫描：选好每天几点。小鹅通填店铺 app_id、B 站填 UP mid，多个用逗号或换行分隔；也可在「站点」页再添加一条同类型站点，分别命名。约牛用已有 Cookie 即可。扫描只看当天发布的内容，已拉取过的地址会跳过。
